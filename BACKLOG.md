...
- [ ] Add Last Stand one-time save from a killing blow: add `G.lastStandUsed` (init `false` in `newGame()`); in `stepEnemies`, wherever core damage would drop `c.hp` to ≤ 0 (bomber detonation and enemy-hits-core paths) and `!G.lastStandUsed`, clamp `c.hp` to 1 instead, set `G.lastStandUsed = true`, and return a `lastStand: true` result flag; in `main.js`, when `result.lastStand` is true, push a dramatic white screen flash FX and show "LAST STAND" via `#waveflash` for 1.5s. Done = a unit test confirms a lethal hit with `lastStandUsed = false` clamps `c.hp` to 1 and sets the flag; a unit test confirms a second lethal hit with `lastStandUsed = true` reduces `c.hp` to 0 normally (no double-save).
