...
<!-- ── 2026-07-03 review batch ────────────────────────────────── -->
- [x] Cap enemy speed growth: export `CONFIG.enemySpeedCap = 160` from `engine.js`; in `derive()`, clamp `enemySpeed` to `Math.min(CONFIG.baseEnemySpeed + wave * CONFIG.enemySpeedScale, CONFIG.enemySpeedCap)`. Done = `CONFIG.enemySpeedCap = 160` exported with a test pinning the value; a test confirms `derive(stats, 100).enemySpeed === derive(stats, 200).enemySpeed` (both clamped to the cap); existing wave-scaling tests (wave 5 > wave 1) still pass. Added CONFIG.enemySpeedCap=160; derive() clamps enemySpeed; 2 new tests (190 total).

- [x] Add Last Stand one-time save from a killing blow: add `G.lastStandUsed` (init `false` in `newGame()`); in `stepEnemies`, wherever core damage would drop `c.hp` to ≤ 0 (bomber detonation and enemy-hits-core paths) and `!G.lastStandUsed`, clamp `c.hp` to 1 instead, set `G.lastStandUsed = true`, and return a `lastStand: true` result flag; in `main.js`, when `result.lastStand` is true, push a dramatic white screen flash FX and show "LAST STAND" via `#waveflash` for 1.5s. Done = a unit test confirms a lethal hit with `lastStandUsed = false` clamps `c.hp` to 1 and sets the flag; a unit test confirms a second lethal hit with `lastStandUsed = true` reduces `c.hp` to 0 normally (no double-save). Added applyCoreDamage() helper wrapping both core-damage sites in stepEnemies; G.lastStandUsed init in newGame(); white flash + "LAST STAND" banner in main.js; 2 new tests (192 total).

- [x] Add Resonance passive skill (Nova + Vortex synergy): add `SKILLS.resonance` (`passive: true`) to `engine.js`; in `stepNovaDet`, when `'resonance'` is unlocked, for each enemy hit by the nova detonation, count other living enemies within 90px at hit time — if 2 or more, multiply that enemy's nova damage by `CONFIG.resonanceMult = 1.5`. Done = `SKILLS.resonance` offerable before unlock and not after (2 tests); `CONFIG.resonanceMult = 1.5` exported with a test pinning the value; a unit test confirms an enemy with 2+ neighbors within 90px takes 1.5× nova damage when resonance is unlocked, and normal damage when it is not. Added SKILLS.resonance + CONFIG.resonanceMult=1.5; neighbor-count damage multiplier in stepNovaDet; nova pool-size test fixed for 19-skill pool; 5 new tests (197 total).

- [x] Add first-launch skill coach mark: on the very first game session (`localStorage` flag `coreSeenCoach`, checked/set in `newGame()` or on start), overlay a pulsing arrow pointing at the skill bar with the text "TAP TO FIRE"; the overlay uses `pointer-events: none` so it never blocks input; it disappears the moment the player's first skill activates (`onSkillTap` fires) and the flag is set permanently in `localStorage`. Done = the coach mark is visible in the browser on a fresh `localStorage` (no `coreSeenCoach` key) and does not appear on subsequent sessions; rendering/input-only, no engine test needed. Added #coach element + CSS bounce animation in index.html; maybeShowCoach()/hideCoach() in main.js triggered from choose() and onSkillTap(); verified live in browser (shows on skill unlock, hides + persists flag on first tap).

- [x] Add haptic tick on skill fire and core hit: in `onSkillTap()`, after a skill fires successfully, call `navigator.vibrate(15)` guarded by `'vibrate' in navigator`; in the `coreHit` handling block in `step()`, call `navigator.vibrate([30, 20, 30])` under the same guard. Done = vibration calls are visible via a devtools override/mock in the browser when a skill fires and when the core takes a hit; guarded so desktop browsers without the Vibration API are unaffected; rendering/input-only, no engine test needed. Added navigator.vibrate calls in onSkillTap() and the coreHit block, both guarded by 'vibrate' in navigator; verified live in browser via a mocked navigator.vibrate (skill tap → 15, core hit → [30,20,30]).
