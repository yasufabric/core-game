...
- [ ] Cap enemy speed growth: export `CONFIG.enemySpeedCap = 160` from `engine.js`; in `derive()`, clamp `enemySpeed` to `Math.min(CONFIG.baseEnemySpeed + wave * CONFIG.enemySpeedScale, CONFIG.enemySpeedCap)`. Done = `CONFIG.enemySpeedCap = 160` exported with a test pinning the value; a test confirms `derive(stats, 100).enemySpeed === derive(stats, 200).enemySpeed` (both clamped to the cap); existing wave-scaling tests (wave 5 > wave 1) still pass.
